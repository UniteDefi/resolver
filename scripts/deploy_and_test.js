...
class ComprehensiveTest {
    ...
    async deployRelayerContracts() {
        ...
        for (const [chainKey, chain] of Object.entries(chains)) {
            ...
            const deployer = new ethers_1.ethers.Wallet(DEPLOYER_PRIVATE_KEY, provider);
            console.log(`Deployer: ${deployer.address}`);
            try {
                // Deploy MockRelayer
                const factory = new ethers_1.ethers.ContractFactory(MOCK_RELAYER_ABI, MOCK_RELAYER_BYTECODE, deployer);
                ...
                const relayerWallet = new ethers_1.ethers.Wallet(RESOLVER_PRIVATE_KEY, provider);
        ...
        // Save deployments
        const deploymentsPath = path_1.default.join(__dirname, "../mock_relayer_deployments.json");
        fs_1.default.writeFileSync(deploymentsPath, JSON.stringify(this.report.deployments, null, 2));
    ...
    async runCrossChainSwap() {
        ...
        const baseProvider = new ethers_1.ethers.JsonRpcProvider(chains.baseSepolia.rpcUrl);
        const arbitrumProvider = new ethers_1.ethers.JsonRpcProvider(chains.arbitrumSepolia.rpcUrl);
        const userWalletBase = new ethers_1.ethers.Wallet(USER_PRIVATE_KEY, baseProvider);
        const userWalletArbitrum = new ethers_1.ethers.Wallet(USER_PRIVATE_KEY, arbitrumProvider);
        console.log(`\n👤 User Address: ${userWalletBase.address}`);
        // Check initial balances
        const srcToken = new ethers_1.ethers.Contract(chains.baseSepolia.tokens.USDT, ERC20_ABI, userWalletBase);
        const dstToken = new ethers_1.ethers.Contract(chains.arbitrumSepolia.tokens.DAI, ERC20_ABI, userWalletArbitrum);
        ...
        console.log(`\n💰 Initial Balances:`);
        console.log(`Base Sepolia USDT: ${ethers_1.ethers.formatUnits(srcBalanceBefore, 6)}`);
        console.log(`Arbitrum Sepolia DAI: ${ethers_1.ethers.formatUnits(dstBalanceBefore, 6)}`);
        ...
        const swapAmount = ethers_1.ethers.parseUnits("10", 6); // 10 USDT
        console.log(`\n✍️ Approving ${ethers_1.ethers.formatUnits(swapAmount, 6)} USDT to relayer...`);
        ...
        console.log(`\n🔐 Order Details:`);
        console.log(`Secret: ${ethers_1.ethers.hexlify(secret)}`);
        ...
        this.report.orderDetails = {
            ...
            srcToken: "USDT",
            dstToken: "DAI",
            amount: ethers_1.ethers.formatUnits(swapAmount, 6),
        ...
        const swapRequest = {
            ...
            secretHash: secretHash,
            minAcceptablePrice: ethers_1.ethers.parseUnits("9.5", 6).toString(),
            orderDuration: 300
        };
        console.log(`\n📤 Creating swap order via API...`);
        try {
            // Check if relayer is running
            await axios_1.default.get(`${RELAYER_URL}/health`);
            // Create order
            const createResponse = await axios_1.default.post(`${RELAYER_URL}/api/create-swap`, {
                swapRequest,
                secret: ethers_1.ethers.hexlify(secret)
            });
            ...
            console.log(`Market Price: ${ethers_1.ethers.formatUnits(createResponse.data.marketPrice, 6)} DAI`);
            this.report.orderDetails.orderId = orderId;
            this.report.orderDetails.marketPrice = ethers_1.ethers.formatUnits(createResponse.data.marketPrice, 6);
            // Monitor order
            ...
            console.log(`Base Sepolia USDT: ${ethers_1.ethers.formatUnits(srcBalanceAfter, 6)}`);
            console.log(`Arbitrum Sepolia DAI: ${ethers_1.ethers.formatUnits(dstBalanceAfter, 6)}`);
            ...
            this.report.orderDetails.result = {
                srcSpent: ethers_1.ethers.formatUnits(srcSpent, 6),
                dstReceived: ethers_1.ethers.formatUnits(dstReceived, 6),
    ...
    async monitorOrderStatus(orderId) {
        ...
        while ((Date.now() - startTime) < 120000) { // 2 min timeout for testing
            ...
            try {
                const statusResponse = await axios_1.default.get(`${RELAYER_URL}/api/order-status/${orderId}`);
    ...
    async generateReport() {
        ...
        // Save full report
        const reportPath = path_1.default.join(__dirname, "../test_report_" + Date.now() + ".json");
