...
fn set_deployed_at(timelocks: u256, timestamp: u64) -> u256 {
...
fn get_deployed_at(timelocks: u256) -> u32 {
    (timelocks >> DEPLOYED_AT_OFFSET).try_into().unwrap()
}

fn src_withdrawal(timelocks: u256) -> u32 {
    (timelocks & TIMELOCK_MASK).try_into().unwrap()
}

fn src_public_withdrawal(timelocks: u256) -> u32 {
    ((timelocks >> 32) & TIMELOCK_MASK).try_into().unwrap()
}

fn src_cancellation(timelocks: u256) -> u32 {
    ((timelocks >> 64) & TIMELOCK_MASK).try_into().unwrap()
}

fn src_public_cancellation(timelocks: u256) -> u32 {
    ((timelocks >> 96) & TIMELOCK_MASK).try_into().unwrap()
}

fn dst_withdrawal(timelocks: u256) -> u32 {
    ((timelocks >> 128) & TIMELOCK_MASK).try_into().unwrap()
}

fn dst_public_withdrawal(timelocks: u256) -> u32 {
    ((timelocks >> 160) & TIMELOCK_MASK).try_into().unwrap()
}

fn dst_cancellation(timelocks: u256) -> u32 {
