module unite::escrow {
    ...
    /// Create generic coin escrow for source chain
    public fun create_coin_escrow_src<T>(
        order_hash: vector<u8>,
        hashlock: vector<u8>,
        maker: address,
        taker: address,
    ...
    /// Create generic coin escrow for destination chain
    public fun create_coin_escrow_dst<T>(
        order_hash: vector<u8>,
        hashlock: vector<u8>,
        maker: address,
        taker: address,
    ...
    /// Add additional resolver to existing generic coin escrow
    public fun add_resolver_to_coin_escrow<T>(
    ...
    /// Deposit generic tokens into escrow (for destination escrows)
    ...
    /// Mark user funds as transferred for generic coin escrow (for source escrows)
    public fun mark_user_funded_coin<T>(escrow: &mut CoinEscrow<T>) {
        escrow.state.user_funded = true;
    }
    ...
    /// Withdraw with secret for generic coin escrow (permissionless)
    public fun withdraw_coin_with_secret<T>(
        escrow: &mut CoinEscrow<T>,
        secret: vector<u8>,
        clock: &Clock,
        ctx: &mut TxContext
        ...
        if (escrow.state.is_source) {
            distribute_source_coin_funds(escrow, caller_reward, ctx);
        } else {
            distribute_destination_coin_funds(escrow, caller_reward, ctx);
        };
    ...
    /// Cancel generic coin escrow
    public fun cancel_coin_escrow<T>(
        escrow: &mut CoinEscrow<T>,
        clock: &Clock,
        ctx: &mut TxContext
    ) {
    ...
    fun distribute_source_coin_funds<T>(
        escrow: &mut CoinEscrow<T>,
        caller_reward: u64,
        ctx: &mut TxContext
    ) {
    ...
    fun distribute_destination_coin_funds<T>(
        escrow: &mut CoinEscrow<T>,
        caller_reward: u64,
        ctx: &mut TxContext
    ) {
    ...
    // === View Functions for Generic Coin Escrows ===
    
    public fun get_coin_escrow_info<T>(escrow: &CoinEscrow<T>): (
        vector<u8>, // order_hash
        vector<u8>, // hashlock
        address,    // maker
        address,    // taker
    ...
    public fun get_coin_resolver_count<T>(escrow: &CoinEscrow<T>): u64 {
        vector::length(&escrow.resolvers)
    }
    
    public fun get_coin_resolver_info<T>(escrow: &CoinEscrow<T>, index: u64): (address, u64, u64, bool) {
        let resolver_info = vector::borrow(&escrow.resolvers, index);
        (
            resolver_info.resolver,
            resolver_info.partial_amount,
    ...
    public fun get_coin_token_balance<T>(escrow: &CoinEscrow<T>): u64 {
        balance::value(&escrow.token_balance)
    }
    
    public fun get_coin_safety_deposits_balance<T>(escrow: &CoinEscrow<T>): u64 {
        balance::value(&escrow.safety_deposits)
    }
}