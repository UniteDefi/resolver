 ...
 * @dev ACTUAL implementation of the relayer contract for cross-chain swaps
 * NOT A MOCK - This handles real user fund transfers using pre-approvals
 */
contract RelayerContract is Ownable, ReentrancyGuard {
     ...
     * This is the ACTUAL implementation - not a mock!
     */
    function transferUserFundsToEscrow(
        ...
        // This uses the actual transferFrom with user's approval
        bool success = IERC20(order.srcToken).transferFrom(
            order.user,
            escrowAddress,
            order.amount
        );
