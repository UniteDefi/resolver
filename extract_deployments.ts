...
Object.entries(deployments).forEach(([chainId, contracts]) => {
    ...
    deploymentsWithNames[`${chainId}`] = {name: chainName, ...contracts}
})

// Write to file
fs.writeFileSync(path.join(__dirname, 'all_deployments.json'), JSON.stringify(deploymentsWithNames, null, 2))
