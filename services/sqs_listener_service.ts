...
export class SQSListenerService {
  ...
  private messageHandler: ((message: SQSOrderMessage) => Promise<void>) | null = null;
  ...
  constructor() {
    ...
    console.log("[SQS Listener] Initialized SQS client for region:", sqsConfig.region);
  }
  
  async initialize(): Promise<void> {
    try {
      // For resolver, we just need the queue URL
      // Queue should already exist (created by relayer)
      const queueUrl = await this.getQueueUrl();
      this.queueUrl = queueUrl;
      console.log("[SQS Listener] Connected to queue:", this.queueUrl);
    } catch (error) {
      console.error("[SQS Listener] Failed to initialize:", error);
      throw error;
    }
  }
  
  private async getQueueUrl(): Promise<string> {
    // Construct queue URL based on region and account
    // In production, you'd get this from environment or API call
    ...
    return `https://sqs.${region}.amazonaws.com/${accountId}/${this.queueName}`;
  }
  
  setMessageHandler(handler: (message: SQSOrderMessage) => Promise<void>): void {
  ...
  private async pollMessages(): Promise<void> {
    ...
    if (response.Messages && response.Messages.length > 0) {
      console.log(`[SQS Listener] Received ${response.Messages.length} messages`);
  ...
  private async processMessage(message: Message): Promise<void> {
    try {
      ...
      console.log(`[SQS Listener] Successfully processed order ${orderMessage.orderId}`);
  ...
  calculateCurrentPrice(
    ...
    const currentPrice = startBn - (priceDiff * BigInt(Math.floor(progress * 1000)) / 1000n);
